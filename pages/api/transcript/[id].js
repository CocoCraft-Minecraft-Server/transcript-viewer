...
export async function getServerSideProps({ params }) {
  try {
    await connectDB();
    const transcript = await Transcript.findById(params.id).lean();
...
function Attachment({ att }) {
  ...
  if (isImg) return (
    <a href={att.url} target="_blank" rel="noopener noreferrer">
      <img src={att.url} alt={name}
        style={{ maxWidth: 400, maxHeight: 300, borderRadius: 4, display: 'block', marginTop: 8, cursor: 'pointer' }} />
    </a>
  );
  if (isVid) return (
  ...
  if (isAud) return (
    <audio src={att.url} controls style={{ display: 'block', marginTop: 8, width: '100%', maxWidth: 400 }} />
  );
  return (
    <div style={{ marginTop: 8, padding: '8px 12px', background: '#2b2d31', borderRadius: 4, display: 'inline-block' }}>
      <a href={att.url} target="_blank" rel="noopener noreferrer"
        style={{ color: '#00b0f4', textDecoration: 'none' }}>📎 {name}</a>
    </div>
  );
}
...
function Message({ msg, prevMsg }) {
  ...
  return (
    <div style={{
      display: 'flex', gap: 16, padding: sameAuthor ? '2px 16px 2px 72px' : '16px 16px 2px 16px',
      ...
      <div style={{ flex: 1, minWidth: 0 }}>
        {!sameAuthor && (
          <div style={{ display: 'flex', alignItems: 'baseline', gap: 8, marginBottom: 2 }}>
            ...
            {msg.isBot && (
              <span style={{
                background: '#5865f2', color: '#fff', fontSize: 10,
                padding: '1px 5px', borderRadius: 3, fontWeight: 700, letterSpacing: 0.5,
              }}>BOT</span>
            )}
        ...
        {msg.embeds?.map((emb, i) => (
          <div key={i} style={{
            marginTop: 8, borderLeft: `4px solid #${emb.color?.toString(16).padStart(6, '0') || '4f545c'}`,
            background: '#2b2d31', borderRadius: '0 4px 4px 0', padding: '8px 12px', maxWidth: 500,
          }}>
    ...
    <>
      <style>{`
        * { box-sizing: border-box; margin: 0; padding: 0; }
        body { background: #313338; font-family: 'gg sans', 'Noto Sans', 'Helvetica Neue', Helvetica, Arial, sans-serif; }
        ::-webkit-scrollbar { width: 8px; }
        ::-webkit-scrollbar-track { background: #2b2d31; }
        ::-webkit-scrollbar-thumb { background: #1a1b1e; border-radius: 4px; }
      `}</style>

      {/* Header tipo servidor Discord */}
      <div style={{ background: '#1e1f22', borderBottom: '1px solid #1a1b1e', padding: '12px 20px', display: 'flex', alignItems: 'center', gap: 12, position: 'sticky', top: 0, zIndex: 10 }}>
        {t.guildIcon && (
          <img src={t.guildIcon} alt={t.guildName}
